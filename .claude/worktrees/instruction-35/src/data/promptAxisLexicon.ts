...
/**
 * 지시문 35 (TASK B) — 랩 딜리버리 어휘(data/rapVocalDelivery.ts의
 * flow/articulation/tone 축) 추가. 새 축을 만들지 않는다 — leadVocal 축의
 * 어휘를 넓힌다. 전부 소문자 고정 문구(includesAny가 클로즈를 lower로 바꾼
 * 뒤 비교하는데 phrase 자체는 그대로 정규식화되므로 대문자가 섞이면
 * 매칭되지 않는다 — 기존 'lead'/'duet'도 같은 이유로 소문자). layering 축
 * (ad-lib stack 등)은 여기 넣지 않는다 — ad-lib류는 backing 성격이 강해
 * BACKING_VOCAL_MARKERS와 충돌할 수 있다(§하지 말 것).
 */
