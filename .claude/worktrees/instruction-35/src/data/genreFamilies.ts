...
export const GENRE_FAMILIES: GenreFamily[] = [
  ...
  // 지시문 35 (TASK A-3) — kr-2030 workspace의 랩 계열 family. 지시문
  // 원문은 이 family의 id를 'family-rap'으로 예시했으나, 그 접두사는 이
  // 파일과 완전히 다른 레지스트리인 data/paletteFamilies.ts의
  // PaletteFamily.id 전용 네임스페이스다(PaletteFamily.id는 전부
  // 'family-'로 시작하고 GenreFamily.id는 이 파일 전체에서 단 하나도
  // 그렇지 않다 — 실측 확인). 'family-rap'을 그대로 쓰면 두 레지스트리의
  // id 네임스페이스가 겹쳐 향후 혼동의 소지가 있어, 이 워크스페이스의
  // 기존 명명 규칙(kr2030-band-emotional/kr2030-night-groove)을 따라
  // 'kr2030-rap-mumble'로 정한다 — memberGenreIds/UI 노출 등 기능은
  // 지시문이 요구한 것과 동일, 이름만 기존 관례에 맞춘다.
  // 신규 8종 + 기존에 이미 kr-2030-pop archetype으로 등록된 랩·힙합 4종
  // (chill-rap/boom-bap-mellow/jazz-rap/trap-soul, presets.ts의
  // after-work-band-pop preferredGenres에 이미 배정돼 있던 값)을 함께
  // 묶는다 — 12종, TASK C의 "12종 이상" preferredGenres 요구와 일치.
  {
