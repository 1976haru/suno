...
import { checkKr2030RapSafety } from './kr2030RapSafetyPolicy';
...
const CATEGORY_BY_ITEM_ID: Record<string, ReleaseReadinessCategory> = {
  ...
  [RELEASE_READINESS_ITEM_IDS.kr2030RapSafetyBlocking]: 'safety',
  [RELEASE_READINESS_ITEM_IDS.kr2030RapSafetyAdvisory]: 'safety'
};
...
export interface ReleaseReadinessInput {
  ...
  archetype?: string;
  /**
   * 지시문 35 (TASK D) — channel.id. archetype만으로는 kr-2030-pop
   * archetype을 공유하는 4개 채널(after-work-band-pop 등 기존 3개 +
   * kr-2030-rap)을 구분할 수 없다 — kr2030UnexpectedRap 검사(랩 없는
   * 채널에 랩 강제 금지)는 나머지 3개 채널에는 여전히 유효하지만
   * kr-2030-rap에는 랩이 이 채널의 정체성이라 그대로 적용하면 전곡이
   * FAIL로 잘못 판정된다(실측: 지시문 35 도입 전 findUnexpectedRapSections는
   * "kr-2030에 랩이 아예 배선되지 않았다"는 전제로 작성됨 — 그 전제가
   * kr-2030-rap 도입으로 깨졌다). 이 채널 전용 안전 정책도 channelId로
   * 스코프한다(archetype으로 하면 나머지 3개 채널에도 적용돼 버린다).
   * Optional: 이 필드를 넘기지 않는 기존 호출부는 이전과 동일하게 동작.
   */
  channelId?: string;
  /**
...
function newItems(input: ReleaseReadinessInput): ReleaseReadinessItem[] {
  const { songs, duplicationHistory, lyricLanguage, archetype, channelId, conceptLabel, explorationTrackNos } = input;
  ...
  if (archetype === 'kr-2030-pop') {
    ...
    // 지시문 35 (TASK D) — kr-2030-rap은 랩이 채널 정체성 그 자체라 이
    // 검사(랩 없는 채널에 랩 강제 금지)의 전제가 성립하지 않는다.
    // channelId로 이 채널만 제외 — archetype 전체를 끄면 나머지 3개 채널
    // (after-work-band-pop 등, forbiddenCliches에 'excessive rap verses'를
    // 명시한 채널들)의 실제 방어가 사라진다.
    if (channelId !== 'kr-2030-rap') {
      ...
      items.push({
        id: RELEASE_READINESS_ITEM_IDS.kr2030UnexpectedRap, categoryKo: '가사', labelKo: 'kr-2030 예상 밖 랩 섹션 없음',
        status: 'not-measured', detail: 'kr-2030-rap 채널 전용 — 랩이 채널 정체성이라 이 검사 대상 아님'
      });
    }
  ...
  // 지시문 35 (TASK D) — core/kr2030RapSafetyPolicy.ts, kr-2030-rap 채널
  // 전용. archetype(kr-2030-pop)이 아니라 channelId로 스코프한다 — 같은
  // archetype을 쓰는 나머지 3개 채널(after-work-band-pop 등)에는 적용하지
  // 않는다(§C-1 "새 아키타입을 만들지 않는다"의 대가로 archetype 단위
  // 정책은 자동 상속되지만, 채널 전용 정책은 이렇게 channelId로 직접
  // 스코프해야 한다).
  if (channelId === 'kr-2030-rap') {
    ...
    items.push({
      ...
      detail: blockingIssues.length ? blockingIssues.map(s => `T${s.trackNo}: ${s.blocking.join(', ')}`).join(' / ') : '없음'
    });
    ...
    items.push({
      ...
      status: advisoryIssues.length === 0 ? 'pass' : 'fail',
      detail: advisoryIssues.length ? advisoryIssues.map(s => `T${s.trackNo}: ${s.advisory.join(', ')}`).join(' / ') : '없음'
    });
  }
