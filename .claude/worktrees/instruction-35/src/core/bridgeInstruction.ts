...
/**
 * 지시문 35 (TASK D-3) — kr-2030-rap 전용 안전 브릿지 지시문. 유튜브
 * 플레이리스트용 채널이라 core/kr2030RapSafetyPolicy.ts의 blocking
 * 카테고리(폭력·무기·약물·범죄, 욕설·비속어, 집단 비하, 실존 인물·브랜드
 * 실명)를 생성 시점에 1차로 막는다 — kr2030RapSafetyPolicy.ts는 생성된
 * 결과를 사후 검사하는 쪽이고, 이 함수는 애초에 그런 가사가 나오지 않게
 * 요청 단계에서 명시한다(§D-1 "유튜브 수익화 정책과 직결된다"). channel.id로
 * 스코프한다 — 같은 archetype(kr-2030-pop)을 쓰는 다른 3개 채널에는 필요
 * 없는 안내다.
 */
...
export function buildClaudeCodeInstruction(
  ...
  return [
    ...
    // 지시문 35 (TASK D-3) — kr-2030-rap 전용 안전 브릿지 지시문, intent
    // 바로 다음(가장 눈에 띄는 위치)에 배치. 다른 모든 채널은 빈 배열.
    ...kr2030RapSafetyBridgeLines(opts),
