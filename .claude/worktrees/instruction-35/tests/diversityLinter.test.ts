...
describe('v3.14 diversity linter — moneyChord', () => {
  ...
  it('reports passed=true and covers all 16 known presets (8 original + 5 v3.33 Part C channel-signature + 3 v3.38 Part B4 kids presets)', () => {
    const report = lintPresetDiversity('moneyChord');
    expect(report.passed).toBe(true);
    expect(report.totalPresets).toBeGreaterThanOrEqual(16);
  });
});
