...
describe('[지시문 16 TASK B-4] classifyClause — real fixture clauses', () => {
  ...
  it('classifies rap delivery clauses (지시문 35 TASK B) as leadVocal', () => {
    expect(classifyClause('dragged vowels, swallowed consonants', false)).toBe('leadVocal');
    expect(classifyClause('mumbled delivery over a triplet flow', false)).toBe('leadVocal');
    expect(classifyClause('whisper-close tone, close-mic breathy', false)).toBe('leadVocal');
    expect(classifyClause('autotuned melodic tone', false)).toBe('leadVocal');
  });

  it('does NOT misclassify unrelated rhythm/instrument clauses that merely share a word with rap delivery vocabulary', () => {
    expect(classifyClause('trap hi-hat rolls', false)).not.toBe('leadVocal');
    expect(classifyClause('sliding 808 bass', false)).not.toBe('leadVocal');
  });
