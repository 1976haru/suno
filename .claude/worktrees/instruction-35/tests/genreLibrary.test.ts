...
describe('structured genre library', () => {
  it('adds 249 Notion-derived genres and keeps legacy genre ids available', () => {
    ...
    // 지시문 35 (TASK A) — kr2030GenrePacks grew 8 -> 16 (+8 kr2030-rap-*
    // ids, registered in both genreLibrary and presets.ts's rawGenrePacks,
    // same pattern).
    expect(genrePacks.length).toBe(LEGACY_IDS.length + importedGenreCount + 27 + 34 + 8 + 8 + 7 + 7 + 7 + 7);
  ...
  it('preserves all genre ids and keeps preset ids backward compatible', () => {
    ...
    // 지시문 35 (TASK A) — +8 rap-* ids (mumble-melodic/whisper-trap/
    // emo-sing-rap/laidback-alt-rnb/drill-dark/boom-bap-modern/cloud-hazy/
    // female-melodic), same registered-in-both pattern: 370.
